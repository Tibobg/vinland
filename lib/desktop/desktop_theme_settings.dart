...
import '../theme/cover_picker.dart';
import '../theme/solid_color_effect.dart';
import 'desktop_theme.dart';
import 'glass.dart';

/// Section "Apparence" des Parametres (desktop uniquement) : choix du theme
/// de fond + palette RGB pour le theme solide.
class DesktopThemeSettingsSection extends StatelessWidget {
  const DesktopThemeSettingsSection({super.key});

  @override
  Widget build(BuildContext context) {
    return Selector<AppState,
        (DesktopThemeMode, Color, double, SolidColorEffect, String?)>(
      selector: (_, state) => (
      ...
      builder: (context, data, __) {
        ...
        return Column(
          ...
          children: [
              ...
              child: Row(
                children: [
                  _ThemeOption(
                    ...
                    iconColor: color,
                    selected: mode == DesktopThemeMode.solid,
                    onTap: () =>
                        state.setDesktopThemeMode(DesktopThemeMode.solid),
                  ),
                  ...
                  _ThemeOption(
                    ...
                    onTap: () =>
                        state.setDesktopThemeMode(DesktopThemeMode.coverBlur),
                  ),
                  const SizedBox(width: 12),
                  _ThemeOption(
                    label: 'Transparent',
                    icon: Icons.blur_circular,
                    selected: mode == DesktopThemeMode.transparent,
                    onTap: () =>
                        state.setDesktopThemeMode(DesktopThemeMode.transparent),
                  ),
                ],
              ),
            ),
            ...
            if (mode == DesktopThemeMode.solid)
              Padding(
                padding: const EdgeInsets.fromLTRB(16, 16, 16, 0),
                child: _SolidColorSection(
                  color: color,
                  onChanged: (c) => state.setDesktopThemeColor(c),
                  effect: effect,
                  onEffectChanged: (e) => state.setDesktopSolidEffect(e),
                ),
              ),
            if (mode == DesktopThemeMode.coverBlur) ...[
              Padding(
                padding: const EdgeInsets.fromLTRB(16, 8, 16, 0),
                child: _BlurSigmaSection(
                  sigma: blurSigma,
                  onChanged: (v) => state.setDesktopCoverBlurSigma(v),
                ),
              ),
              Padding(
                ...
                child: PinnedCoverSection(
                  ...
                  albums: state.albums,
                  onChanged: (path) => state.setDesktopPinnedCoverPath(path),
                ),
              ),
            ],
          ],
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 10,
          runSpacing: 10,
          children: DesktopTheme.presetColors.map((preset) {
